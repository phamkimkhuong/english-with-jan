...
ZIP_PATH="${1:-/home/ubuntu/english-with-jan-stt-service.zip}"
...
fail() {
  printf '[stt-deploy] ERROR: %s\n' "$*" >&2
  exit 1
}

require_root() {
  if [ "$(id -u)" -ne 0 ]; then
    fail "Run as root, for example: sudo $0 $ZIP_PATH"
  fi
}
...
validate_release() {
  ...
  [ -f "$release_dir/app/main.py" ] || fail "Release is missing app/main.py"
  [ -f "$release_dir/deploy/oracle/stt-service.service" ] || fail "Release is missing systemd template"
}

sync_release() {
  local release_dir="$1"

  log "Syncing release into $APP_DIR"
  rsync -a --delete \
    --exclude='.env' \
    --exclude='.venv/' \
    --exclude='models/' \
...
main() {
  ...
  validate_release "$TMP_DIR/release"

  sync_release "$TMP_DIR/release"
  ensure_venv_and_dependencies
  install_systemd_unit
  install_convenience_commands
